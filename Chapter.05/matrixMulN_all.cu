...
void initializeMatrices(double *matrix_a, double *matrix_b, int N) {
    srand(time(NULL));
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
...
void showResults(double *matrix_a, double *matrix_b, double *matrix_c, int N) {
    printf("***** MATRIX A *****\n");
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            (j % N == N - 1) ? printf("%f\n", matrix_a[i*N + j]) : printf("%f,", matrix_a[i*N + j]);
        }
    }
    printf("***** MATRIX B *****\n");
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            (j % N == N - 1) ? printf("%f\n", matrix_b[i*N + j]) : printf("%f,", matrix_b[i*N + j]);
        }
    }
    printf("***** MATRIX C *****\n");
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            (j % N == N - 1) ? printf("%f\n", matrix_c[i*N + j]) : printf("%f,", matrix_c[i*N + j]);
        }
    }
}
